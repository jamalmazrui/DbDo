...
A simulated walk through DbDo, made with AI: a person working, and the reader answering.

**Before you start:** DbDo is installed and your reader is running. Nothing else is needed.

### Step 1: Insert+UpArrow

One key before we start. If a line goes by too fast, Insert plus Up Arrow says the line again, as often as you like. Every walk in this series assumes you have it.
...
- I am John. I speak what the reader says.
...
Opening a database and moving through it: what a row says, what a cell says, and how to reach the next table.
...
The menus: how an item says its key and its letter, and how to leave without choosing anything.
...
Adding a record: the form, the pick list in a field, and saving with one key.
...
Typing a letter jumps to the first value starting with it, so L reaches lead.
...
Looking at one record closely: the whole record at once, and the keys that say a single field.
...
Changing a value: opening the record, editing one cell, and checking what was saved.
...
Two ways to reach a record: typing a few letters to jump, and searching every field for a word.
...
Deciding what you hear: the order of the list, and shortening it to the records that matter.
...
Typing S jumps to the first field starting with it.
...
Choosing which fields a row speaks, and hearing the choice you made.
...
Following a record to the records that belong to it, and coming back.
...
Marking records to work with a few of them, and clearing the marks afterwards.
...
The two computed fields every table has: what a record is called, and what makes it unique.
...
Getting records out: copying a row, writing a report, and saving under a new name.
...
A whole task end to end: recording what you did, then running the report that proves it.
...
Typing W jumps to the report I want.
